...
const setup = async ({
  ...
  fetchMock.post("path:/api/ee/cloud-add-ons/metabase-ai-tiered", 200);
...
describe("MetabotPurchasePage", () => {
  ...
  it("submits a HTTP request", async () => {
    ...
    await userEvent.click(
      screen.getByRole("radio", { name: /up to 1500 requests/ }),
    );
    await userEvent.click(
      screen.getByRole("checkbox", { name: /Terms of Service/ }),
    );
    ...
    expect(cloudAddOnsRequest?.body?.quantity).toEqual(1500);
